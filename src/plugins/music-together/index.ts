...
>({
  name: () => 'Music Together',
  description: () => 'Listen to music together with friends in real time.',
  restartNeeded: false,
  addedVersion: '3.2.X',
  config: {
    enabled: false,
  ...
  renderer: {
    ...
    async onHost() {
      ...
      this.connection.onConnections((connection) => {
        ...
        if (!connection.open) {
          this.api?.toastService?.show(
            'Music Together: User disconnected',
          );
      ...
      const listener = async (event: ConnectionEventUnion) => {
        ...
        switch (event.type) {
          ...
          case 'PERMISSION': {
            ...
            const permissionLabel = 'Set Permission';

            this.api?.toastService?.show(
              'Music Together: Permission changed',
            );
            break;
          }
          case 'CONNECTION_CLOSED': {
        ...
        onItemClick: (id) => {
          ...
          if (id === 'music-together-copy-id') {
            navigator.clipboard
              ...
              .then(() => {
                this.api?.toastService?.show(
                  'Music Together: ID copied',
                );
                hostPopup.dismiss();
              })
              .catch(() => {
                this.api?.toastService?.show(
                  'Music Together: Failed to copy ID',
                );
                hostPopup.dismiss();
              });
          }
      ...
      const settingPopup = createSettingPopup({
        onItemClick: async (id) => {
          ...
          if (id === 'music-together-join') {
            ...
            if (result) {
              this.api?.toastService?.show(
                'Music Together: Joined',
              );
              guestPopup.showAtAnchor(setting);
            } else {
              this.api?.toastService?.show(
                'Music Together: Failed to join',
              );
            }
          }
        },
