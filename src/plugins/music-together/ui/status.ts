...
export const createStatus = () => {
  ...
  const setStatus = (status: 'disconnected' | 'host' | 'guest') => {
    ...
    if (status === 'host') {
      statusLabel.textContent = 'Host';
      statusLabel.style.color = 'rgba(255, 0, 0, 1)';
    }

    if (status === 'guest') {
      statusLabel.textContent = 'Guest';
