...
async function downloadSongUnsafe(
  ...
  console.info(
    `Downloading ${metadata.artist} - ${metadata.title}`,
  );
  ...
  return await ffmpegMutex.runExclusive(async () => {
    try {
      ...
      ffmpeg.setProgress(({ ratio }) => {
        sendFeedback(
          `Conversion progress: ${Math.floor(ratio * 100)}%`,
          ratio,
        );
...
export async function downloadPlaylist(givenUrl?: string | URL) {
  ...
  if (!playlistId) {
    sendError(
      new Error('Playlist ID not found.'),
    );
    return;
  }
  ...
  try {
    ...
    sendError(
      Error(
        `Error getting playlist: ${String(error)}`,
      ),
    );
    return;
  }
  ...
  if (existsSync(playlistFolder)) {
    if (!config.skipExisting) {
      sendError(
        new Error(
          `Folder already exists: ${playlistFolder}`,
        ),
      );
      return;
    }
  ...
  dialog.showMessageBox(win, {
    ...
    title: 'Start Download Playlist',
    message: `Starting download for playlist: ${playlistTitle}`,
    detail: `Playlist size: ${items.length}`,
  });
  ...
  try {
    for (const song of items) {
      sendFeedback(
        `Downloading song ${counter}/${items.length}`,
      );
      ...
      await downloadSongFromId(
        ...
        sendError(
          new Error(
            `Error downloading song: ${song.author!.name} - ${song.title!}, Error: ${String(error)}`,
          ),
        ),
      );
